...
main_df = pd.read_csv("lijst2_final_withdistractors.csv", delimiter=";")
...
while not consent_given:
    intro_text = ("Welkom bij het online experiment van ons onderzoek!\n"
                  "Wij willen je vragen om anoniem deel te nemen aan ons online wetenschappelijk experiment. \n" 
                  ...
                  "Deelname aan het experiment duurt ongeveer 5-10 minuten. Er zijn geen nadelen of risico's aan verbonden. \n"
...
for koppel_id, unit_df in unit_list:
    ...
    if total_sentences > 65 and (total_sentences - len(unit_df)) <= 65:
    ...
    if total_sentences > 120 and (total_sentences - len(unit_df)) <= 120:
        break_screen_text = "Je kunt nu een korte pauze nemen. Als je klaar bent om door te gaan, druk dan op de spatiebalk."
        break_screen = visual.TextStim(win, text=break_screen_text, color="white", pos=(0, 0), height=0.07)
        break_screen.draw()
        win.flip()
...
output_folder = "experiment 2"
os.makedirs(output_folder, exist_ok=True)  # Create 'experiment 2' folder if it doesn't exist
