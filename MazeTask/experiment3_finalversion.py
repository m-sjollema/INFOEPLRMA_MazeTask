...
main_df = pd.read_csv("lijst3_final_withdistractors.csv", delimiter=";")
...
for koppel_id, unit_df in unit_list:
    ...
    if total_sentences > 80 and (total_sentences - len(unit_df)) <= 80:
    ...
    if total_sentences > 155 and (total_sentences - len(unit_df)) <= 155:
        break_screen_text = "Je kunt nu een korte pauze nemen. Als je klaar bent om door te gaan, druk dan op de spatiebalk."
        break_screen = visual.TextStim(win, text=break_screen_text, color="white", pos=(0, 0), height=0.07)
        break_screen.draw()
        win.flip()
...
output_folder = "experiment 3"
os.makedirs(output_folder, exist_ok=True)  # Create 'experiment 3' folder if it doesn't exist
